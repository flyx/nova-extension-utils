...
If you don't know how overlays work, just use the code from the template that gives you `pkgs`.
For simple usage, you only need to modify the arguments given to these two functions.
...
The only special cases are `config` and `configWorkspace`, which are processed by `nova-extension-utils`, see below.
...
Instead of defining a full `key` for every option, you give it a `name`.
The configuration value's `key` will be calculated by concatenating your extension's `identifier`, the names of any `type = section` ancestors, and your item's `name`.
...
"config": [
  {
    ...
    "children": [
      {
        "key": "org.example.languageServer.path",
        "title": "Path",
        "type": "path",
        "default": "nil"
      }
...
In your `main.js`, you can query configuration values from the autogenerated `config.js`:

```javascript
const Config = require("config.js").Config;
let config = new Config();
config.languageServer.path.value(); // retrieve the current value
```

This will retrieve the workspace config value or, if unset, the global value.
